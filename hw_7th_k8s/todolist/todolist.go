...
func load_database() {
	// 讀取config.json資料
	file, err := os.Open("/etc/config/config.json")
	if err != nil {
	...
	db, err := db_conn.DB()
	if err != nil {
		mistake_control(err, 87, "db_conn.DB()")
		return
	}
	...
	// 更新資料庫資料
	err = db_conn.Debug().AutoMigrate(&Todolist{})
	if err != nil {
		mistake_control(err, 93, "db_conn.Debug().AutoMigrate")
		return
	}
